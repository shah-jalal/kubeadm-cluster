...
# Determine the latest Kubernetes version
...
# Update apt package index, install kubeadm, kubectl, kubelet
apt-get update
apt-get install -y kubelet kubeadm kubectl
apt-mark hold kubelet kubeadm kubectl

# Configure crictl in case we need it to examine running container
crictl config \
